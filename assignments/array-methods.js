...
runners.forEach((run) => {
    fullName.push({"first_name": run.first_name, "last_name": run.last_name});
}); 
...
const allCaps = runners.map((name) => {
    return {"first_name": name.first_name.toUpperCase(), "last_name": name.last_name.toUpperCase()};
});
...
let largeShirts = runners.filter ((largers) => {
    return largers.shirt_size === 'L';
...
let ticketPriceTotal = runners.reduce((accumulator, donate) => {
        accumulator = accumulator + donate.donation;
    return accumulator
}, 0);
// COMPLETE
...
// Problem 1: Find donors who give more than $100 to send them a donor gift!
let bigGivers = runners.filter ((big) => {
    return big.donation >= 100;
     ;
});

// COMPLETE
console.log(bigGivers);

// Problem 2 Convert donations to strings
const tostring = runners.map((name) => {
    return {"donate": name.donation.toString()};
});

// COMPLETE
console.log(tostring);

// Problem 3: create new array of emails

let email = []

runners.forEach((run) => {
    email.push({"email": run.email});
}); 
